      ...
      instrument      <- read.csv("district-analysis/data/clean/instrument/instrument_forex_dist.csv")
      census_outcome  <- read.csv("district-analysis/data/clean/census/outcomes_district.csv")
      source("district-analysis/script/estimation_display.R")
      
      census_panel <- census_outcome %>%
        left_join(instrument, by = c("dname", "year")) %>%
        left_join(
          instrument %>% mutate(year = year + 1) %>% 
            rename_with(~ paste0(., "_lag1"), -c(dname, year)),
          by = c("dname", "year")
        ) %>%
        left_join(
          instrument %>% mutate(year = year + 2) %>% 
            rename_with(~ paste0(., "_lag2"), -c(dname, year)),
          by = c("dname", "year")
        ) %>%
        mutate(
          across(
      ...
      etable(m1, m2, m3, m4, m5, m6, m7, cluster = ~dname)
      ...
      etable(m1, m2, m3, m4, m5, m6, cluster = ~dname)
      ...
      etable(m1, m2, m3, m4, m5, m6, m7, m8, m9, cluster = ~dname)
