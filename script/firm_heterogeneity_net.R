# =============================================================================
# script/firm_heterogeneity_net.R
#
# Firm-level heterogeneity by NET INTERNAL MIGRATION (2001 baseline).
#   RECEIVER = net-inflow muni    (in-mig share >= out-mig intensity)
#   SENDER   = net-outflow muni   (in-mig share <  out-mig intensity)
# Classification uses 2001 census in-migrant share minus 2001 instrument
# out-migration intensity -- predetermined w.r.t. all shock years.
#
...
#
# Output: output/tab/firm_heterogeneity_net.csv
#
# Run from repo root:  source("script/firm_heterogeneity_net.R")
# =============================================================================
...
# -----------------------------------------------------------------------------
# 3.  Net-migration classification (2001 baseline)
#     RECEIVER = mig_in_share_2001 >= mig_intensity_2001  (in >= out)
#     SENDER   = mig_in_share_2001 <  mig_intensity_2001  (in <  out)
# -----------------------------------------------------------------------------
cen_raw <- fread("data/clean/census/census_outcomes_municipality.csv")
in_2001 <- cen_raw[year == 2001 & !is.na(mig_in_share), .(lgcode, mig_in_share)]
out_2001 <- inst[year == 2001 & !is.na(mig_intensity), .(lgcode, mig_intensity)]
...
cls <- cls[, .(lgcode, sample)]
cat(sprintf("Net-migrant lookup: %d receivers, %d senders, %d total\n",
            cls[sample == "receiver", .N], cls[sample == "sender", .N], nrow(cls)))
...
nec_p <- merge(nec_p, cls, by = "lgcode", all.x = FALSE)
cat(sprintf("NEC panel: %d rows, %d munis (%d receivers, %d senders)\n",
            nrow(nec_p), uniqueN(nec_p$lgcode),
            uniqueN(nec_p[sample=='receiver']$lgcode),
            uniqueN(nec_p[sample=='sender']$lgcode)))
...
nec_cs <- merge(nec_cs, cls, by = "lgcode", all.x = FALSE)
cat(sprintf("NEC cs:    %d munis (%d receivers, %d senders), %d districts\n",
            nrow(nec_cs),
            nec_cs[sample=='receiver', .N], nec_cs[sample=='sender', .N],
            uniqueN(nec_cs$DIST)))
...
# -----------------------------------------------------------------------------
# 6.  Outcome catalogues  (same as firm_rural_urban.R)
# -----------------------------------------------------------------------------
...
NEC_CS_OUTCOMES <- c(
  ...
  "share_trade_retail","share_hospitality","share_finance_prof_info",
  "share_social_services","share_transport","share_other_services"
)
NEC_PANEL_OUTCOMES <- intersect(NEC_PANEL_OUTCOMES, names(nec_p))
NEC_CS_OUTCOMES    <- intersect(NEC_CS_OUTCOMES,    names(nec_cs))

# -----------------------------------------------------------------------------
# 7.  Estimators (same code as firm_rural_urban.R)
# -----------------------------------------------------------------------------
...
fit_panel <- function(panel, outcome, thr, ref_year, fe = "lgcode + year") {
  ...
  if (nrow(d) < 50 || uniqueN(d[[outcome]]) < 2 || sd(d[[outcome]], na.rm = TRUE) == 0)
    return(NULL)
  d <- copy(d); d[, treatment := fx_z * log_migint_z]
...
SAMPLES <- list(full = NULL, receiver = "receiver", sender = "sender")
...
out_path <- file.path(ROOT, "output/tab/firm_heterogeneity_net.csv")
fwrite(out, out_path)
cat("\nSaved: ", normalizePath(out_path, winslash = "/"), "  (", nrow(out), " rows)\n", sep = "")

# -----------------------------------------------------------------------------
# 9.  Summary at thr=25 (anchor)
# -----------------------------------------------------------------------------
cat("\n========== NEC PANEL: log(# new firms) by net-mig sample (thr = 25) ==========\n")
print(out[dataset == "nec_panel" & threshold == 25,
          .(outcome, sample,
            ...
            n, n_muni)],
      nrows = 60)
cat("\n========== NEC CS 2018: muni-level outcomes by net-mig sample (thr = 25) ==========\n")
print(out[dataset == "nec_cs" & threshold == 25,
          .(outcome, sample,
            beta = signif(beta, 4), stars,
            beta_pp = signif(beta_pp, 4), pct = signif(pct_of_mean, 4),
            se = signif(se, 3), pval = signif(pval, 3),
