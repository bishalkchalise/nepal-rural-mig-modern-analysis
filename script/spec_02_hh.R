# =============================================================================
# SPEC 02 — HRVS household panel.  Main saturated spec MINUS the second term
# (year x mig_intensity trend), threshold = 25 only.
#
...
#        + alpha_hh + gamma_t + e_it                          # HH + year FE
#
# Dropped vs the main A4 spec:
#   sum_t lambda_{1,t} * mig_int_z * 1{year==t}               # C_mig    (REMOVED)
#
# Sample:    HRVS HH x year (2016, 2017, 2018), ref year 2016
# Threshold: 25 (munis with >= 25 migrants in 2001 ONLY)
# Cluster:   lgcode (municipality)
# Treatments z-scored on the muni-year working sample.
#
# Outputs: output/tab/spec_02_hh_results.csv (one row per outcome).
#
# Run from repo root in RStudio:  source("script/spec_02_hh.R")
# Or:                              Rscript script/spec_02_hh.R
# =============================================================================
...
cat(sprintf("Baseline X covariates: %d (%s)\n",
            length(BASELINE_X_COLS), paste(BASELINE_X_COLS, collapse = ", ")))

# --- 3. Load HH master (agriculture base + merge other domains) ---------------
...
# --- 5. Build the working panel at threshold=25 ------------------------------
REF_YEAR  <- 2016L
THRESHOLD <- 25L
...
# Re-z-score on muni-year working sample
...
run_one <- function(d, y) {
  ...
  fml <- as.formula(sprintf("%s ~ %s | hhid + year",
...
for (i in seq_along(ALL_OUTCOMES)) {
  ...
  base <- data.table(
    ...
    spec = "spec_01", threshold = THRESHOLD,
...
# --- 7. Save + console summary ----------------------------------------------
dir.create(file.path(ROOT, "output/tab"), recursive = TRUE, showWarnings = FALSE)
out_path <- file.path(ROOT, "output/tab/spec_02_hh_results.csv")
fwrite(out, out_path)
