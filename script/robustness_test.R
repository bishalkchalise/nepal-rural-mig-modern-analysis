# =============================================================================
# script/robustness_test.R
#
# Slim, RStudio-safe test sweep — same output schema as robustness_all.R but
# only the 4 most diagnostic specs at k=25.  Use this first; if it completes
# cleanly, you can step up to robustness_all.R (12 specs × 4 thresholds).
#
# Specs:
#   S0_baseline      — current anchor
#   S1_cmig_log      — year × log(mig_int_z) instead of linear (over-control test)
#   S8_no_cmig       — drop year × mig_int_z (your over-controlling hypothesis)
#   S11_FE_only      — strip every year-interacted control (bare FE)
#
# Threshold: 25 only.  Outcomes: every numeric column across all four datasets.
#
# Output: output/tab/robustness_test.csv
# Wall-clock: ~15–25 min.  Crash-tolerant: writes CSV after each spec completes.
# =============================================================================
...
# ----- Outcome catalogues (same as robustness_all.R) ------------------------
...
# ----- Group + interpret helpers (same logic as robustness_all.R) -----------
...
detect_scale <- function(outcome, mean_y) {
  ...
  if (grepl("(_rs$|_rs_|_amt$|spend_|food_exp_|nonfood_|cost_per_|^cap_|crop_sale_rs|earning_total_rs|cost_12m_rs|land_sold_12m_rs)", outcome))
    return("rs")
  if (!is.na(mean_y) && abs(mean_y) <= 1 && mean_y >= 0) return("share")
  if (grepl("(diversity|hhi|index|score|n_industries)", outcome)) return("index")
  if (grepl("(^n_|count|firms_|emp_total|^new_firms)", outcome)) return("count")
  "other"
}
interpret_beta <- function(beta, outcome, mean_y) {
  ...
  if (scale == "log")    sprintf("%+.1f%% change (log outcome)", beta*100)
  else if (scale == "rs")    sprintf("%+s Rs%s",
                                     formatC(round(beta), format="d", big.mark=","),
                                     pct_of_mean())
  else if (scale == "share") sprintf("%+.2fpp%s", beta*100, pct_of_mean())
  else if (scale == "index") sprintf("%+.3f index pts (baseline %.3f)", beta, mean_y)
  else if (scale == "count") sprintf("%+.3f units%s", beta, pct_of_mean())
  else                       sprintf("%+.3f (baseline %.3f)", beta, mean_y)
}

# ----- Loop with INCREMENTAL save (crash-tolerant) --------------------------
outcs <- build_outcomes()
out_path <- file.path(ROOT, "output/tab/robustness_test.csv")
dir.create(file.path(ROOT, "output/tab"), recursive = TRUE, showWarnings = FALSE)

all_rows <- list(); t_start <- Sys.time()
cat(sprintf("\n========== robustness_test (slim) ==========\n"))
...
for (spec_name in names(SPECS)) {
  cfg <- SPECS[[spec_name]]
  cat(sprintf("--- %s (lag=%d, c_mig_log=%s, c_mig=%s, c_fx=%s, blockA=%s) ---\n",
              spec_name, cfg$lag, cfg$c_mig_log, cfg$c_mig, cfg$c_fx, cfg$c_block_a))
  # census via run_spec
  ...
  for (y in NEC_PANEL_OUTCOMES) {
    fit <- fit_nec_panel(y, cfg$lag, cfg$c_mig_log, cfg$c_mig, cfg$c_fx, cfg$c_block_a, THRESHOLD)
  ...
  for (y in NEC_CS_OUTCOMES) {
    fit <- fit_nec_cs(y, cfg$lag, cfg$c_mig_log, cfg$c_mig, cfg$c_fx, cfg$c_block_a, THRESHOLD)
