# =============================================================================
# script/robustness_fx_alone.R
#
# fx_alone spec across all four datasets at k>=25.
#
# Spec:
#   Treatment: fx_z  (destination-share-weighted FX shock; the SSIV by itself)
#   Controls:  year × mig_int_z (linear, trend on overall migration intensity)
#              + Block A × year (destination characteristics)
#              + entity FE + year FE
#   Drop:      year × fx_z   (would be collinear with the treatment)
#
# Compare:
#   S0_baseline  (current anchor: fx_z × log(mig_int_z), full control set)
#   S_fx_alone   (this spec)
#
# Output: output/tab/robustness_fx_alone.csv  — same schema (outcome_group,
#         interpret) as robustness_test.csv.
...
SPECS <- list(
  S0_baseline = list(treatment = "log_int", c_mig = TRUE, c_fx = TRUE,  c_block_a = TRUE),
  S_fx_alone  = list(treatment = "fx_alone", c_mig = TRUE, c_fx = FALSE, c_block_a = TRUE)
)
THRESHOLD <- 25L
...
# ----- NEC panel + cs setup (identical to previous scripts) -----
...
# ----- Helpers (zscore, year dummies, FEs) -----
zscore <- function(x) {
  s <- sd(x, na.rm = TRUE)
...
fit_nec_panel <- function(outcome, treatment_type, c_mig, c_fx, c_block_a, threshold) {
  ...
  if (nrow(d) < 50 || uniqueN(d[[outcome]]) < 2 || sd(d[[outcome]], na.rm = TRUE) == 0)
    return(NULL)
  d <- copy(d)
  if (treatment_type == "fx_alone")  d[, treatment := fx_z]
...
fit_nec_cs <- function(outcome, treatment_type, c_mig, c_fx, c_block_a, threshold, cluster = "DIST") {
  ...
  # If treatment == fx_z, can't include fx_z again in controls — drop the redundant column
  rhs <- unique(rhs)
  if (treatment_type == "fx_alone" && "fx_z" %in% rhs) rhs <- setdiff(rhs, "fx_z")
...
# ----- Group + interpret helpers (carried over from previous scripts) -----
classify_outcome <- function(o) {
  fcase(
    ...
    grepl("(share_self|share_fallow|share_both|crop_|grows_horticulture|n_crops|effective_n_crops|horti_value_share|cashcrop_value_share|share_irr|share_rented|n_plots_rented|land_sold|any_crop_sold|crop_sale)", o), "Land use / cropping",
...
out_path <- file.path(ROOT, "output/tab/robustness_fx_alone.csv")
dir.create(file.path(ROOT, "output/tab"), recursive = TRUE, showWarnings = FALSE)

all_rows <- list(); t_start <- Sys.time()
cat("========== robustness_fx_alone ==========\n")
cat(sprintf("Specs: %d (S0_baseline + S_fx_alone)\n", length(SPECS)))
...
save_partial <- function() {
  ...
  KEEP <- c("outcome_group","dataset","outcome","spec","threshold","treatment_type",
...
for (spec_name in names(SPECS)) {
  cfg <- SPECS[[spec_name]]
  cat(sprintf("--- %s (treatment=%s, c_mig=%s, c_fx=%s, blockA=%s) ---\n",
              spec_name, cfg$treatment, cfg$c_mig, cfg$c_fx, cfg$c_block_a))

  # census via run_spec
  r <- run_quiet(spec_label = spec_name, dataset = "census",
                 threshold = THRESHOLD, treatment = cfg$treatment,
                 c_mig = cfg$c_mig, c_mig_log = FALSE, c_fx = cfg$c_fx,
                 c_block_a = cfg$c_block_a, c_block_b = FALSE, c_block_c = FALSE,
                 outcomes = list(robust = outcs$census), save = FALSE, lag = 0L)
  if (!is.null(r) && nrow(r) > 0) {
    r[, treatment_type := cfg$treatment]
    all_rows[[length(all_rows)+1]] <- r
  }

  # hh via run_spec
  r <- run_quiet(spec_label = spec_name, dataset = "hh",
                 threshold = THRESHOLD, treatment = cfg$treatment,
                 c_mig = cfg$c_mig, c_mig_log = FALSE, c_fx = cfg$c_fx,
                 c_block_a = cfg$c_block_a, c_block_b = FALSE, c_block_c = FALSE,
  ...
  for (y in NEC_PANEL_OUTCOMES) {
    fit <- fit_nec_panel(y, cfg$treatment, cfg$c_mig, cfg$c_fx, cfg$c_block_a, THRESHOLD)
    base <- data.table(dataset = "nec_panel", outcome = y, spec = spec_name,
                       threshold = THRESHOLD, treatment_type = cfg$treatment)
  ...
  for (y in NEC_CS_OUTCOMES) {
    fit <- fit_nec_cs(y, cfg$treatment, cfg$c_mig, cfg$c_fx, cfg$c_block_a, THRESHOLD)
  ...
  cat(sprintf("  done %s — %.1f min elapsed.  Writing partial CSV…\n",
              spec_name, as.numeric(difftime(Sys.time(), t_start, units = "mins"))))
