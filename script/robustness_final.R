# =============================================================================
# script/robustness_final.R
#
# Final robustness package — one CSV with the headline spec and 8 alternatives.
#
# Spec list (9 total per outcome × threshold):
#   S0_baseline      — fx × log(mig_int_z); c_mig = year × mig_int_z (linear)
#   S_lag1..S_lag10  — same as S0 but treatment uses FX shifter lagged 1/2/3/4/5/10 yrs
#   S_both_log       — fx × log(mig_int_z); c_mig = year × log(mig_int_z)
#                      ('consistent log' — same object both sides)
#   S_both_linear    — fx × mig_int_z (linear share); c_mig = year × mig_int_z (linear)
#                      ('consistent linear' — same object both sides)
#
# All four datasets at k = 25 (THRESHOLDS = c(25L) by default; uncomment line
# in section 1 to run all four thresholds).  Crash-tolerant: writes the CSV
# after each spec completes.
#
# Output: output/tab/robustness_final.csv  with outcome_group + interpret
# Wall-clock at k=25: ~50–60 min.
# =============================================================================
...
# -----------------------------------------------------------------------------
# 4. NEC fitters — parameterised on treatment + lag + c_mig_log
# -----------------------------------------------------------------------------
zscore <- function(x) {
  s <- sd(x, na.rm = TRUE)
...
make_treatment <- function(d, treatment_kind) {
  ...
  else if (treatment_kind == "fx_alone") d[, treatment := fx_z]
  else                                   d[, treatment := fx_z * log_migint_z]
  d
}

fit_nec_panel <- function(outcome, treatment_kind, lag_L, c_mig_log, threshold = 25L) {
...
fit_nec_cs <- function(outcome, treatment_kind, lag_L, c_mig_log, threshold = 25L, cluster = "DIST") {
...
# -----------------------------------------------------------------------------
# 5. Outcome group + interpretation helpers
# -----------------------------------------------------------------------------
