# =============================================================================
# script/robustness_levels.R
#
# Level-only controls: keep mig_int_z and fx_z as LEVELS rather than year-
# interacted dummies.  Spec:
#   Treatment: fx_z × log(mig_int_z)
#   Controls:  fx_z (level)  +  mig_int_z (level, absorbed by entity FE in panels)
#              + Block A × year + entity FE + year FE
# Drop:        year × fx_z   and   year × mig_int_z
#
# Compares against S0_baseline (the full year-interacted spec).
#
# Output: output/tab/robustness_levels.csv  — same schema as previous tests.
...
SPECS <- list(
  S0_baseline = list(spec_kind = "S0",       desc = "year × mig_int_z + year × fx_z + Block A × year + FE"),
  S_levels    = list(spec_kind = "levels",   desc = "fx_z (level) + Block A × year + FE [no year×X dummies]")
)
THRESHOLD <- 25L
...
# Generic NEC panel fit; spec_kind controls whether year × X dummies or just levels
fit_nec_panel_levels <- function(outcome, spec_kind, threshold = 25L) {
  ...
  if (spec_kind == "S0") {
    ...
    # mig_int_z absorbed by muni FE; include fx_z level and Block A × year
    for (k in BLOCK_A_COLS)
      year_cols <- c(year_cols, build_year_dummies_nec(d, k, paste0("cA_", k), 2001L))
    rhs <- c("treatment", "fx_z", year_cols)
  }
...
# NEC cs spec is identical between S0 and levels (no year interactions possible at T=1)
fit_nec_cs_levels <- function(outcome, threshold = 25L, cluster = "DIST") {
...
# For census/HH we need a "levels-only" run.  The engine has c_mig and c_fx
# toggles that switch year-interaction on/off — but not a "level only"
# option.  Implement standalone fit for census + HH with levels.
fit_census_or_hh_levels <- function(dataset, outcome, threshold = 25L) {
  if (dataset == "census") {
    df_master <- load_census()
    fe <- c("lgcode","year")
    cluster_var <- "lgcode"
  } else {
    df_master <- load_hh()
  ...
  inst_use <- inst[, .(lgcode, year, fxshock, mig_intensity, total_migrants)]
  inst_use[, log_mig_intensity := log(mig_intensity + 1e-8)]
  df <- merge(df_master, inst_use, by = c("lgcode","year"), suffixes = c("",".inst"))
  df <- df[total_migrants >= threshold]
  if (nrow(df) < 50) return(NULL)
  muni_yr <- unique(df[, .(lgcode, year, fxshock, mig_intensity, log_mig_intensity)])
  muni_yr[, fx_z         := zscore(fxshock)]
  muni_yr[, mig_int_z    := zscore(mig_intensity)]
  muni_yr[, log_migint_z := zscore(log_mig_intensity)]
  df <- merge(df, muni_yr[, .(lgcode, year, fx_z, mig_int_z, log_migint_z)],
              by = c("lgcode","year"))
  if (!is.null(BLOCK_A)) df <- merge(df, BLOCK_A, by = "lgcode", all.x = TRUE)
  d <- df[!is.na(get(outcome)) & !is.na(fx_z)]
  if (nrow(d) < 50 || uniqueN(d[[outcome]]) < 2 || sd(d[[outcome]], na.rm = TRUE) == 0)
    return(NULL)
  d <- copy(d)
  d[, treatment := fx_z * log_migint_z]
  ...
  for (k in BLOCK_A_COLS)
    year_cols <- c(year_cols, build_year_dummies_nec(d, k, paste0("cA_", k), 2001L))
  rhs <- c("treatment", "fx_z", year_cols)   # mig_int_z absorbed by entity FE
  fml <- as.formula(sprintf("%s ~ %s | %s", outcome, paste(rhs, collapse = " + "),
                            paste(fe, collapse = " + ")))
  fit <- tryCatch(feols(fml, data = d, cluster = as.formula(paste0("~", cluster_var)),
                        notes = FALSE, warn = FALSE),
                  error = function(e) e)
  if (inherits(fit, "error")) return(list(err = substr(conditionMessage(fit), 1, 120)))
  cf <- coef(fit); se_ <- se(fit); pv <- pvalue(fit)
...
# ----- Outcome group + interpretation helpers -----
classify_outcome <- function(o) {
  fcase(
    ...
    grepl("^owns_|cost_seed|cost_fert|cost_labour|input_cost|total_input", o), "Input use / capital",
    grepl("(share_self|share_fallow|share_both|crop_|grows_horti)", o), "Land use / cropping",
    ...
    o %in% c("formality_index","share_registered","share_tax_registered","industry_diversity",
             "share_female_led","share_keeps_accounts"), "Firm structure (NEC 2018)",
    default = "Other"
  )
}
interpret_beta <- function(b, o, m) {
  if (is.na(b)) return("")
  scale <- if (grepl("^log_", o)) "log"
           else if (grepl("(_rs$|spend_|food_exp_|nonfood_)", o)) "rs"
           else if (!is.na(m) && abs(m) <= 1 && m >= 0) "share"
           else "other"
  pct <- if (!is.na(m) && abs(m) > 1e-9) sprintf(" (%+.0f%% of mean)", 100*b/m) else ""
  if (scale == "log") sprintf("%+.1f%% (log)", b*100)
  else if (scale == "rs") sprintf("%+s Rs%s", formatC(round(b), format="d", big.mark=","), pct)
  else if (scale == "share") sprintf("%+.2fpp%s", b*100, pct)
  else sprintf("%+.3f (mean %.3f)", b, m)
}

# ----- Loop -----
outcs <- build_outcomes()
all_rows <- list(); t_start <- Sys.time()
out_path <- file.path(ROOT, "output/tab/robustness_levels.csv")
dir.create(file.path(ROOT, "output/tab"), recursive = TRUE, showWarnings = FALSE)

cat("========== robustness_levels ==========\n")
cat(sprintf("Specs: S0_baseline (year-interacted) vs S_levels (levels only)\n"))
cat(sprintf("Outcomes: census=%d hh=%d nec_panel=%d nec_cs=%d  (k>=%d)\n\n",
            length(outcs$census), length(outcs$hh),
            length(NEC_PANEL_OUTCOMES), length(NEC_CS_OUTCOMES), THRESHOLD))
...
save_partial <- function() {
  ...
  KEEP <- c("outcome_group","dataset","outcome","spec","threshold",
  ...
  fwrite(d[order(outcome_group, dataset, outcome, spec)], out_path)
}

# Spec 1: S0_baseline (use run_spec with the standard flags)
cat("--- S0_baseline (year-interacted controls) ---\n")
r <- run_quiet(spec_label = "S0_baseline", dataset = "census",
               threshold = THRESHOLD, treatment = "log_int",
               c_mig = TRUE, c_mig_log = FALSE, c_fx = TRUE, c_block_a = TRUE,
...
r <- run_quiet(spec_label = "S0_baseline", dataset = "hh",
               threshold = THRESHOLD, treatment = "log_int",
               c_mig = TRUE, c_mig_log = FALSE, c_fx = TRUE, c_block_a = TRUE,
               c_block_b = FALSE, c_block_c = FALSE,
...
for (y in NEC_PANEL_OUTCOMES) {
  fit <- fit_nec_panel_levels(y, "S0", THRESHOLD)
  base <- data.table(dataset = "nec_panel", outcome = y, spec = "S0_baseline", threshold = THRESHOLD)
...
for (y in NEC_CS_OUTCOMES) {
  fit <- fit_nec_cs_levels(y, THRESHOLD)
  base <- data.table(dataset = "nec_cs", outcome = y, spec = "S0_baseline", threshold = THRESHOLD)
  if (is.null(fit)) { base[, err := "NULL/degenerate"] }
  else if (!is.null(fit$err)) { base[, err := fit$err] }
  else base[, `:=`(beta=fit$beta, se=fit$se, pval=fit$pval, stars=stars_fn(fit$pval),
                   n=fit$n, n_muni=fit$n_muni, mean_y=fit$mean_y, sd_y=fit$sd_y, err="")]
...
cat(sprintf("  done S0 — %.1f min\n", as.numeric(difftime(Sys.time(), t_start, units = "mins"))))
save_partial()

# Spec 2: S_levels (level-only controls)
cat("--- S_levels (level-only controls; mig_int_z absorbed by FE) ---\n")
for (y in outcs$census) {
  fit <- fit_census_or_hh_levels("census", y, THRESHOLD)
  base <- data.table(dataset = "census", outcome = y, spec = "S_levels", threshold = THRESHOLD)
  if (is.null(fit)) { base[, err := "NULL/degenerate"] }
  else if (!is.null(fit$err)) { base[, err := fit$err] }
  else base[, `:=`(beta=fit$beta, se=fit$se, pval=fit$pval, stars=stars_fn(fit$pval),
                   n=fit$n, n_muni=fit$n_muni, mean_y=fit$mean_y, sd_y=fit$sd_y, err="")]
...
for (y in outcs$hh) {
  fit <- fit_census_or_hh_levels("hh", y, THRESHOLD)
  base <- data.table(dataset = "hh", outcome = y, spec = "S_levels", threshold = THRESHOLD)
  if (is.null(fit)) { base[, err := "NULL/degenerate"] }
  else if (!is.null(fit$err)) { base[, err := fit$err] }
  else base[, `:=`(beta=fit$beta, se=fit$se, pval=fit$pval, stars=stars_fn(fit$pval),
                   n=fit$n, n_muni=fit$n_muni, mean_y=fit$mean_y, sd_y=fit$sd_y, err="")]
...
for (y in NEC_PANEL_OUTCOMES) {
  fit <- fit_nec_panel_levels(y, "levels", THRESHOLD)
  base <- data.table(dataset = "nec_panel", outcome = y, spec = "S_levels", threshold = THRESHOLD)
  if (is.null(fit)) { base[, err := "NULL/degenerate"] }
  else if (!is.null(fit$err)) { base[, err := fit$err] }
  else base[, `:=`(beta=fit$beta, se=fit$se, pval=fit$pval, stars=stars_fn(fit$pval),
                   n=fit$n, n_muni=fit$n_muni, mean_y=fit$mean_y, sd_y=fit$sd_y, err="")]
...
for (y in NEC_CS_OUTCOMES) {
  fit <- fit_nec_cs_levels(y, THRESHOLD)  # cs spec identical to S0 since no year × X dummies
  base <- data.table(dataset = "nec_cs", outcome = y, spec = "S_levels", threshold = THRESHOLD)
  if (is.null(fit)) { base[, err := "NULL/degenerate"] }
  else if (!is.null(fit$err)) { base[, err := fit$err] }
  else base[, `:=`(beta=fit$beta, se=fit$se, pval=fit$pval, stars=stars_fn(fit$pval),
                   n=fit$n, n_muni=fit$n_muni, mean_y=fit$mean_y, sd_y=fit$sd_y, err="")]
...
cat(sprintf("  done S_levels — %.1f min\n", as.numeric(difftime(Sys.time(), t_start, units = "mins"))))
save_partial()
