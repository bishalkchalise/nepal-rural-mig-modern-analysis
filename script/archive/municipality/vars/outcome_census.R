    #####################################################################################################
    # NEPAL CENSUS PANEL — MUNICIPALITY (LGCODE) LEVEL
    # Produces: data/clean/outcomes_municipality.csv
    #
    # Structure:
    #   - Load each census, attach lgcode
    ...
    #####################################################################################################
    # SECTION 2 — Load 2001 files (attach lgcode via VDC crosswalk)
    #####################################################################################################
    ...
    #####################################################################################################
    # SECTION 3 — Compute 2001 topics (all at lgcode level)
    #####################################################################################################
    ...
    edu_01 <- edu_lit_01 %>%
      full_join(edu_att_01, by = "lgcode") %>%
      full_join(edu_at2_01, by = "lgcode") %>%
      mutate(year = 2001)
    ...
    mar_01 <- mar_base_01 %>% full_join(mar_fem_01, by = "lgcode") %>% mutate(year = 2001)
    ...
    edu_11 <- edu_lit_11 %>%
      full_join(edu_att_11, by = "lgcode") %>%
      full_join(edu_at2_11, by = "lgcode") %>%
      mutate(year = 2011)
    ...
    mar_11 <- mar_base_11 %>% full_join(mar_fem_11, by = "lgcode") %>% mutate(year = 2011)
    ...
    #####################################################################################################
    # SECTION 6 — Load 2021 files (mnid = lgcode directly)
    #####################################################################################################
    ...
    edu_21 <- edu_lit_21 %>%
      full_join(edu_att_21, by = "lgcode") %>%
      full_join(edu_at2_21, by = "lgcode") %>%
      mutate(year = 2021)
    ...
    mar_21 <- mar_base_21 %>% full_join(mar_fem_21, by = "lgcode") %>% mutate(year = 2021)
    ...
    # Join everything at (lgcode, year)
    panel_muni <- amen_df %>%
      full_join(housing_df, by = c("lgcode","year")) %>%
      full_join(femx_df,    by = c("lgcode","year")) %>%
      full_join(shock_df,   by = c("lgcode","year")) %>%
      full_join(edu_df,     by = c("lgcode","year")) %>%
      full_join(mar_df,     by = c("lgcode","year")) %>%
      full_join(fert_df,    by = c("lgcode","year")) %>%
      full_join(work_df,    by = c("lgcode","year")) %>%
      full_join(occ_df,     by = c("lgcode","year")) %>%
      full_join(industry_df,     by = c("lgcode","year")) %>%
      full_join(emp_df,     by = c("lgcode","year")) %>%
      full_join(mig_df,     by = c("lgcode","year")) %>%
      full_join(flfp_df,    by = c("lgcode","year")) %>%
      full_join(head_df,    by = c("lgcode","year")) %>%
      full_join(left_df,    by = c("lgcode","year")) %>%
      arrange(lgcode, year)
